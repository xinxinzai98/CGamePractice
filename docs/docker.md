...
本手册描述可选 Docker 部署方式。2026-09-11 已发布的 v0.3.1 测试版使用 systemd 与 Nginx，未在实验蜂运行 Docker；当前 HTTP 入口、250 项本地测试及公网认证／双客户端 MAGI 验证见[实验蜂部署记录](deployment-shiyanfeng.md)。
...
上述历史 Docker 验证不包含实验蜂主机，也不代表对当前版本重新进行了容器验收。2026-09-11 已另行完成 systemd 部署及公网接口／联机冒烟验证；HTTPS、公网浏览器可视检查、人工长时及全部关卡验收仍未完成。
